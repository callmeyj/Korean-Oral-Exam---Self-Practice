...
export default function App() {
  ...
  async function handleStudentTurn(text) {
    ...
    if (stage === 0) {
      ...
      const sys = `당신은 NSW HSC ${level==="KB"?"Korean Beginners":"Korean Continuers"} 구술 시험관입니다. 학생이 Opening Question에 답했습니다. Extended Question을 하나 하세요. 학생 답변 내용을 더 구체적으로 파고드는 질문입니다 (언제요? 어디에서요? 누구하고요? 얼마나 자주요? 어떻게요?). 규칙: 한국어로만, 질문 하나만, 왜요는 사용하지 마세요, 칭찬 금지, 질문 텍스트만 출력.`;
  ...
  async function askEnrichment(history) {
    const sys = `당신은 NSW HSC ${level==="KB"?"Korean Beginners":"Korean Continuers"} 구술 시험관입니다. 학생이 Extended Question에 답했습니다. Enrichment Question을 하나 하세요. 학생의 의견, 느낌, 이유, 생각을 묻는 질문입니다 (왜 그렇게 생각해요? 어떤 느낌이에요? 어떻게 생각해요?). 규칙: 한국어로만, 질문 하나만, 칭찬 금지, 질문 텍스트만 출력.`;
